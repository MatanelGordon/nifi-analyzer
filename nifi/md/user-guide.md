...
When the application is started, the user is able to navigate to the UI by going to the default address of `https://localhost:8443/nifi` in a web browser. The default configuration generates a username and password with full system administration privileges. For information on securing the system, see the System Administrator’s Guide.
...
* Policy: access users/groups
  * Privilege: Allows users to view and modify the users and user groups
...
!Label **Label**: Labels are used to provide documentation to parts of a dataflow. When a Label is dropped onto the canvas, it is created with a default size. The Label can then be resized by dragging the handle in the bottom-right corner. The Label has no text when initially created. The text of the Label can be added by right-clicking on the Label and choosing `Configure`.
...
##### [](about:blank/user-guide.html#run-schedule)Run Schedule
...
##### [](about:blank/user-guide.html#automatically-retry)Automatically Retry
...
#### [](about:blank/user-guide.html#assigning_parameter_context_to_PG)Assigning a Parameter Context to a Process Group
...
!Parameter Providers Information Buttons
...
##### [](about:blank/user-guide.html#component-access-policies)Component Access Policies
...
#### [](about:blank/user-guide.html#Custom_Properties)Referencing Custom Properties via nifi.properties
...
#### [](about:blank/user-guide.html#configure-site-to-site-server-nifi-instance)Configure Site-to-Site Server NiFi Instance
...
[](about:blank/user-guide.html#navigating)Navigating within a DataFlow
----------------------------------------------------------------------
...
### [](about:blank/user-guide.html#keywords)Keywords
...
### [](about:blank/user-guide.html#process_group_anatomy)Anatomy of a Process Group
...
### [](about:blank/user-guide.html#remote_group_anatomy)Anatomy of a Remote Process Group
...
### [](about:blank/user-guide.html#Queue_Interaction)Queue Interaction
...
### [](about:blank/user-guide.html#Summary_Page)Summary Page
...
### [](about:blank/user-guide.html#Status_History)Historical Statistics of a Component
...
[](about:blank/user-guide.html#versioning_dataflow)Versioning a DataFlow
------------------------------------------------------------------------
...
### [](about:blank/user-guide.html#connecting-to-a-nifi-registry)Connecting to a NiFi Registry
...
Once a Registry Client has been added, the DFM may configure it by clicking the "Edit" button (!Edit Button) in the far-right column. When the DFM clicks the "Edit" button, an Edit Registry Client window opens. It has two tabs: Settings and Properties. The Settings tab provides a place for the DFM to edit the Registry Client name. It also lists the UUID, Type, and provides an open-text field to edit or add a Description.
...
### [](about:blank/user-guide.html#version_states)Version States
...
### [](about:blank/user-guide.html#import-a-versioned-flow)Import a Versioned Flow
...
### [](about:blank/user-guide.html#start-version-control)Start Version Control
...
### [](about:blank/user-guide.html#managing_local_changes)Managing Local Changes
...
#### [](about:blank/user-guide.html#show-local-changes)Show Local Changes
...
#### [](about:blank/user-guide.html#revert-local-changes)Revert Local Changes
...
#### [](about:blank/user-guide.html#commit-local-changes)Commit Local Changes
...
### [](about:blank/user-guide.html#change-version)Change Version
...
### [](about:blank/user-guide.html#stop-version-control)Stop Version Control
...
### [](about:blank/user-guide.html#nested-versioned-flows)Nested Versioned Flows
...
### [](about:blank/user-guide.html#parameters-in-versioned-flows)Parameters in Versioned Flows
...
The GetFile processor in PG1 references the variable "RPG\_Var":
...
the added GetFile processor will also reference the "RPG\_Var" variable that exists in the root process group:

!Both PG1 Reference RPG Variable

If PG1 versioned flow is imported into a different NiFi instance where "RPG\_Var" does not exist:

!PG1 Imported to Different NiFi

A "RPG\_Var" variable is created in the PG1 process group:

!PG1 References PG Variable Copy

### [](about:blank/user-guide.html#Restricted_Components_in_Versioned_Flows)Restricted Components in Versioned Flows
...
*   There are two users, "sys\_admin" and "test\_user" who have access to both view and modify the root process group.
    
*   "sys\_admin" has access to all restricted components.
    
    !Sys_admin Restricted Component Access Policy
    
*   "test\_user" has access to restricted components requiring 'read filesystem' and 'write filesystem'.
...
#### [](about:blank/user-guide.html#restricted-controller-service-created-in-root-process-group)Restricted Controller Service Created in Root Process Group

In this first example, sys\_admin creates a KeytabCredentialsService controller service at the root process group level.
...
Sys\_admin creates a process group ABC containing a flow with GetFile and PutHDFS processors:
...
Sys\_admin saves the process group as a versioned flow:

!ABC Versioned Flow

Test\_user changes the flow by removing the KeytabCredentialsService controller service:

!PutHDFS No Kerberos CS

If test\_user chooses to revert this change:
...
Additionally, if test\_user chooses to import the ABC versioned flow:
...
#### [](about:blank/user-guide.html#restricted-controller-service-created-in-process-group)Restricted Controller Service Created in Process Group

Now, consider a second scenario where the controller service is created on the process group level.

Sys\_admin creates a process group XYZ:

!XYZ Process Group

Sys\_admin creates a KeytabCredentialsService controller service at the process group level:
...
Test\_user changes the flow by removing the KeytabCredentialsService controller service. However, with this configuration, if test\_user attempts to revert this change:

!Test_user Revert Local Changes

the revert is unsuccessful because test\_user does not have the 'access keytab' permissions required by the KeytabCredentialService controller service:

!Revert Local Changes Fails

Similarly, if test\_user tries to import the XYZ versioned flow:
...
[](about:blank/user-guide.html#templates)Templates
--------------------------------------------------
...
### [](about:blank/user-guide.html#Create_Template)Creating a Template
...
### [](about:blank/user-guide.html#Import_Template)Importing a Template
...
### [](about:blank/user-guide.html#instantiating-a-template)Instantiating a Template
...
### [](about:blank/user-guide.html#Manage_Templates)Managing Templates
...
#### [](about:blank/user-guide.html#Export_Template)Exporting a Template

Once a Template has been created, it can be shared with others in the Template Management page. To export a Template, locate the Template in the table. The Filter in the top-right corner can be used to help find the appropriate Template if several are available. Then click the "Download" button (!Export). This will download the template as an XML file to your computer. This XML file can then be sent to others and imported into other instances of NiFi (see Importing a Template).

#### [](about:blank/user-guide.html#removing-a-template)Removing a Template
...
[](about:blank/user-guide.html#data_provenance)Data Provenance
--------------------------------------------------------------
...
### [](about:blank/user-guide.html#provenance_events)Provenance Events

Each point in a dataflow where a FlowFile is processed in some way is considered a 'provenance event'. Various types of provenance events occur, depending on the dataflow design. For example, when data is brought into the flow, a RECEIVE event occurs, and when data is sent out of the flow, a SEND event occurs. Other types of processing events may occur, such as if the data is cloned (CLONE event), routed (ROUTE event), modified (CONTENT\_MODIFIED or ATTRIBUTES\_MODIFIED event), split (FORK event), combined with other data objects (JOIN event), and ultimately removed from the flow (DROP event).

The provenance event types are:



* Provenance Event: ADDINFO
  * Description: Indicates a provenance event when additional information such as a new linkage to a new URI or UUID is added
* Provenance Event: ATTRIBUTES_MODIFIED
  * Description: Indicates that a FlowFile’s attributes were modified in some way
* Provenance Event: CLONE
  * Description: Indicates that a FlowFile is an exact duplicate of its parent FlowFile
* Provenance Event: CONTENT_MODIFIED
  * Description: Indicates that a FlowFile’s content was modified in some way
* Provenance Event: CREATE
  * Description: Indicates that a FlowFile was generated from data that was not received from a remote system or external process
* Provenance Event: DOWNLOAD
  * Description: Indicates that the contents of a FlowFile were downloaded by a user or external entity
* Provenance Event: DROP
  * Description: Indicates a provenance event for the conclusion of an object’s life for some reason other than object expiration
* Provenance Event: EXPIRE
  * Description: Indicates a provenance event for the conclusion of an object’s life due to the object not being processed in a timely manner
* Provenance Event: FETCH
  * Description: Indicates that the contents of a FlowFile were overwritten using the contents of some external resource
* Provenance Event: FORK
  * Description: Indicates that one or more FlowFiles were derived from a parent FlowFile
* Provenance Event: JOIN
  * Description: Indicates that a single FlowFile is derived from joining together multiple parent FlowFiles
* Provenance Event: RECEIVE
  * Description: Indicates a provenance event for receiving data from an external process
* Provenance Event: REMOTE_INVOCATION
  * Description: Indicates that a remote invocation was requested to an external endpoint (e.g., deleting a remote resource)
* Provenance Event: REPLAY
  * Description: Indicates a provenance event for replaying a FlowFile
* Provenance Event: ROUTE
  * Description: Indicates that a FlowFile was routed to a specified relationship and provides information about why the FlowFile was routed to this relationship
* Provenance Event: SEND
  * Description: Indicates a provenance event for sending data to an external process
* Provenance Event: UNKNOWN
  * Description: Indicates that the type of provenance event is unknown because the user who is attempting to access the event is not authorized to know the type


### [](about:blank/user-guide.html#searching-for-events)Searching for Events
...
### [](about:blank/user-guide.html#event_details)Details of an Event
...
### [](about:blank/user-guide.html#replay_flowfile)Replaying a FlowFile
...
### [](about:blank/user-guide.html#viewing-flowfile-lineage)Viewing FlowFile Lineage
...
#### [](about:blank/user-guide.html#find-parents)Find Parents
...
#### [](about:blank/user-guide.html#expanding-an-event)Expanding an Event
...
### [](about:blank/user-guide.html#writeahead-provenance)Write Ahead Provenance Repository
...
#### [](about:blank/user-guide.html#backwards-compatibility)Backwards Compatibility
...
#### [](about:blank/user-guide.html#older-existing-nifi-version)Older Existing NiFi Version
...
#### [](about:blank/user-guide.html#bootstrap-conf)Bootstrap.conf
...
#### [](about:blank/user-guide.html#system-properties)System Properties
...
[](about:blank/user-guide.html#experimental_warning)Experimental Warning
------------------------------------------------------------------------
...
[](about:blank/user-guide.html#other_management_features)Other Management Features
----------------------------------------------------------------------------------
